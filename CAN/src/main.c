...
void CAN1_Tx()
{
	...
	if( HAL_CAN_AddTxMessage(&hcan1,&TxHeader,&message1,&TxMailbox1) != HAL_OK)
	{
		Error_handler();
	}
 ...
 void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
 ...
 void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
 {
	 ...
	 uint8_t message2; //no meaning for data frame
	...
	if ( req_counter  == 4)
	{
		...
		if( HAL_CAN_AddTxMessage(&hcan1,&TxHeader,&message2,&TxMailbox2) != HAL_OK)
		{
			Error_handler();
		}
		req_counter = 0;
